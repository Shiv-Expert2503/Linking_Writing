...
events = ['q', 'Space', 'Backspace', 'Shift', 'ArrowRight', 'Leftclick', 'ArrowLeft', '.', ',', 'ArrowDown', 'ArrowUp',
          'Enter', 'CapsLock', "'", 'Delete', 'Unidentified']
text_changes = ['q', ' ', '.', ',', '\n', "'", '"', '-', '?', ';', '=', '/', '\\', ':']
...
class DataTransformation:
    ...
    @staticmethod
    def test_transformation(test_path):
        try:
            logging.info('Reading Test Data')
            test_logs = pl.scan_csv(test_path)
            test_feats = dev_feats(test_logs)
            test_feats = test_feats.collect().to_pandas()
            logging.info('Read Successfully Now transforming')
            ...
            test_feats = handle_null(test_feats)
            logging.info("Transformation Completed")
            return test_feats
        except Exception as e:
            logging.error("Error while transforming test data", exc_info=True)
            raise CustomException(e, sys)
